...
export function useChecklists() {
  ...
  const addWeek = () => {
    setState(prev => {
      const c       = prev.checklists.find(cl => cl.id === prev.activeChecklistId)
      if (!c) return prev
      const nextNum = (c.weeks[c.weeks.length - 1]?.week ?? c.weeks.length) + 1
      const newWeek = {
        week: nextNum,
        days: [
          { day: 'Monday',    tasks: [] },
      ...
      setActiveWeekIndex(c.weeks.length)   // new week will be at this index
      return {
        ...prev,
        checklists: prev.checklists.map(cl =>
          cl.id !== prev.activeChecklistId ? cl : { ...cl, weeks: [...cl.weeks, newWeek] }
        ),
      }
    })
  }
